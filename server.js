...
console.log("current dir is  " + path.join(__dirname));

//firebase admin setup

let serviceAccount = require("./ecom-website-de265-firebase-adminsdk-aovv7-b4809609d8.json");
...
const bucketName = "stayfit-website";
...
app.post("/signup", (req, res) => {
  ...
  if (name.length < 3) {
    ...
    return res.json({ alert: "password should be 8 letters" });
...
app.post("/add-product", (req, res) => {
  ...
  db.collection("products")
    ...
    .catch((err) => {
      return res.json({ alert: "some error occoured. try again" });
    });
});
...
app.post("/get-products", async (req, res) => {
  ...
  if (id) {
    ...
    tag = tag.charAt(0).toUpperCase() + tag.substr(1, tag.length);
    let someTest = db.collection("products");
    let getSomeTags = await someTest.get();
    let productArr = [];
    getSomeTags.forEach((data) => {
    ...
    if (productArr.length > 0) return res.json(productArr);
    else return res.json("no products");
  } else {
  ...
  db.collection("order")
    ...
    .then((data) => {
      transporter.sendMail(mailOption, (err, info) => {
        if (err) {
          res.json({
            alert: "opps! its seems like some error occured. try again",
          });
        } else {
...
app.post("/my_orders", (req, res) => {
  res.sendFile(path.join(staticPath, "my_orders.html"));
});
